...
exports.apply_promo_code = function (req, res, next) {
    ...
    utils.check_request_params(req.body, [{name: 'user_id', type: 'string'},{name: 'promocode', type: 'string'},
        {name: 'trip_id', type: 'string'}], function (response) {
        if (response.success) {
            User.findOne({_id: req.body.user_id}).then((user) => {
...
exports.getuser_referalcredit = function (req, res, next) {
                    ...
                    if (req.body.token != null && user.token != req.body.token) {
                        ...
                        Country.findOne({countryname: user.country}).then((country) => {
                            if (country) {
                                ...
                                var referral_credit = user.refferal_credit;

                                res.json({
                                    success: true,
                                    message: success_messages.MESSAGE_CODE_YOU_GET_YOUR_REFERAL_CREDIT_SUCCESSFULLY,
                                    currency: currency,
...
exports.get_user_setting_detail = function (req, res) {
    ...
    var terms_and_condition_url = req.protocol + '://' + req.get('host') + "/terms&condition";
    var privacy_policy_url = req.protocol + '://' + req.get('host') + "/privacy";
