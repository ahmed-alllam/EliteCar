...
exports.provider_add_vehicle = function (req, res, next) {
    ...
    utils.check_request_params(req.body, [{name: 'provider_id', type: 'string'},{name: 'vehicle_name', type: 'string'},
        ...
        if (response.success) {
            Provider.findOne({_id: req.body.provider_id}).then((provider) => {
                if (provider) {
                    if (req.body.token != null && provider.token != req.body.token) {
                        ...
                        console.log('res for add car error 1 ---->');
                        ...
                        var files = req.files;
                        Country.findOne({countryname: provider.country}).then((country) => {
                            console.log('res for add car error 2 ---->', x,country);
                            Document.find({countryid: country._id, type: 2}).then((document) => {
                                console.log('res for add car error 3 ---->', document);
                                ...
                                if (document_size !== 0) {
                                    ...
                                    document.forEach(function (entry, index) {
                                        console.log('res for add car error 3.1 ---->', entry);
                                ...
                                console.log('res for add car error 4 ---->', response);
                                provider.vehicle_detail.unshift(vehicel_json);
                                console.log('res for add car error 5 ---->', provider.vehicle_detail);
                                provider.save().then(() => {
                                    console.log('res for add car error 6 ---->', response);
            ...
            console.log('res for add car error ---->', response);
            res.json({
                success: false,
                error_code: response.error_code,
                error_description: response.error_description
