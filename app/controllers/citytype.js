...
exports.list = function (req, res) {
    utils.check_request_params(req.body, [
        ...
        if (response.success) {
			...
			//country=country.replace(/And/gi,'and');
			//country=country.replace(/&/gi,'and');
			console.log(country);
            ...
            table.findOne({_id: id}).then((detail) => {
                ...
                Country.findOne({$or:[{countrycode1:country_code},{countryname:country}], isBusiness: constant_json.YES}).then((country) => {
                       console.log(country);
                    if (!country) {
                         // console.log("--------111-----");
                                            ...
                                            RedZoneArea.find({cityid: city_id}).then((red_zone_area_list)=>{
                                                ...
                                                if(!inside_red_zone){
                                                    CityZone.find({cityid: city_id}).then((zone_list)=>{
                                                        ...
                                                        var buiesness_condotion = {$match: {'is_buiesness': {$eq: 1}}};
                                                        ...
                                                        Citytype.aggregate([countryid_condition, cityid_condition, buiesness_condotion, city_type_to_type_query, array_to_json, rrr, lookup]).then((citytypes) => {
                                                            ...
                                                            // console.log(citytypes);
                                                            citytypes.forEach(function(citytype_data){
                                                                 // console.log("----------city type data-----");
                                                                 // console.log(citytype_data.is_car_rental_business);
                                                                if(citytype_data.is_car_rental_business){
                                                                    ...
                                                                    car_rental_list.forEach(function(car_rental_data){
                                                                        if(car_rental_data.is_buiesness){
                                                                            citytype_data.car_rental_list.push(car_rental_data);
                                                                            // console.log(citytype_data.car_rental_list);
                                                                        }
                                                                    })
                                                                } else {
                                                                    citytype_data.car_rental_list = [];
...
exports.disptcher_city_type_list = function (req, res) {
    ...
    utils.check_request_params(req.body, [
        ...
        if (response.success) {
            ...
            Country.findOne({countryname: subAdminCountry, isBusiness: constant_json.YES}).then((country) => {
                            ...
                            city_details.forEach(function (city_detail) {
                                ...
                                if (count == size) {
                                    ...
                                    if (finalCityId != null) {
                                        ...
                                        var cityid_condition = {$match: {'cityid': {$eq: city_id}}};
                                        var buiesness_condition = {$match: {'is_buiesness': {$eq: 1}}};
...
//                 var country = req.body.country;
//                 Country.findOne({countryname: country, isBusiness: constant_json.YES}).then((country) => {
...
//                     } else {
//                         var currency = country.currencysign;
//                         City.find({isBusiness: constant_json.YES}).then((city_details) => {
