            ...
            if (response) {
                ...
                Partner.findOne({phone: req.body.phone}).then((response) => { 
                    if (response) {
                        ...
                        Partner.findOne({}, function(error, partner_count){

                            if (partner_count) {
                                partnercount = partner_count.unique_id + 1;
                            }
...
exports.get_available_vehicle_list = function (req, res) {
    if (typeof req.session.partner != 'undefined') {
        Partner.findOne({_id: req.body.id}).then((partner) => { 
            if (partner) {
                Citytype.findOne({_id: req.body.service_type_id},function(city_type_detail){

                    var vehicle_array = [];
                    partner.vehicle_detail.forEach(function (vehicle) {
                        if (vehicle.admin_type_id != null && vehicle.is_assigned !== true) {
        ...
        Partner.findOne({_id: req.body.id}).then((partner) => { 
            if (partner) {
                ...
                Provider.findOne({_id: req.body.provider_id}).then((provider) => { 
                    ...
                    partner.vehicle_detail[index].is_selected = true;
    ...
    if (typeof req.session.partner != 'undefined') {
        Partner.findOne({_id: req.body.id}).then((partner) => { 
            if (partner) {
                ...
                Provider.findOne({_id: req.body.provider_id}).then((provider) => { 
                    var vehicle_id = partner.vehicle_detail[0]._id;
                    provider.vehicle_detail = [];
                    provider.service_type = null;
                    provider.admintypeid = null;
                    provider.is_vehicle_document_uploaded = false;
                    provider.save();
                    var vehicle_index = partner.vehicle_detail.findIndex((vehicle) => vehicle._id == vehicle_id)
