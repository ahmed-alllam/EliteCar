            ...
            if (response) {
                ...
                Hotel.findOne({phone: req.body.phone}).then((response) => {
                    ...
                    if (response) {
                        ...
                        var hash = crypto.createHash('md5').update(password).digest('hex');
                        var hotelcount = 1;
                        Hotel.findOne({}).then((hotel_count) => {

                            if (hotel_count) {
                                hotelcount = hotel_count.unique_id + 1;
                            }
...
exports.hotel_update_profile = function (req, res) {
    if (typeof req.session.hotel != "undefined") {
        Hotel.findById(req.body.id).then((hotel_detail) => {
            ...
            {
                Hotel.findByIdAndUpdate(req.body.id).then((resp) => {
...
exports.hotel_future_request = function (req, res) {
    ...
    if (typeof req.session.hotel == 'undefined') {
        ...
        Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, hotel_type_condition, lookup, unwind, search, filter, count]).then((array) => {
            ...
            if (array.length == 0) {
                ...
                Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, hotel_type_condition, lookup, unwind, search, filter, sort, skip, limit]).then((array) => {
    ...
    if (typeof req.session.hotel == 'undefined') {
       ...
       // var unwind1 = {$unwind: "$provider_detail"};
        ...
        //Trip.aggregate([condition, lookup, unwind, lookup1, search, filter, count]).then((array) => {
			Trip.aggregate([condition, lookup,unwind,lookup1,search,filter,count]).then((array) => {
			console.log("hotel request detail")
			//console.log(array);
            if (array.length == 0) {
                ...
                Trip.aggregate([condition, lookup, unwind, lookup1, search, filter, sort, skip, limit]).then((array) => {
					console.log("/*-/*-/*-/*-/*-/-*/*-/*-/")
						console.log(array[1].provider_detail[0].first_name);
...
exports.genetare_hotel_future_request_excel = function (req, res) {
    ...
    if (typeof req.session.hotel == 'undefined') {
        ...
        Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, hotel_type_condition, lookup, unwind, search, filter, sort]).then((array) => {
